...
import traceback
# These imports need to be relative to work as a package
...
class Detection:
    def __init__(self):
        caffemodel = os.path.join(os.path.dirname(__file__), '..', 'resources', 'detection_model', 'Widerface-RetinaFace.caffemodel')
        deploy = os.path.join(os.path.dirname(__file__), '..', 'resources', 'detection_model', 'deploy.prototxt')
    ...
    def get_bbox(self, img):
        ...
        # It's possible no faces are detected, handle this gracefully
        if out.ndim == 1:
            return None
